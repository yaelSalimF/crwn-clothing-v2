...
export function* isUserAuthenticatedAsync() {
  try {
    ...
    if (!userAuth) return;
    yield call(getSnapshotFromUserAuthAsync, userAuth);
  } catch (error) {
    yield put(signInFailed(error));
  }
}
...
export function* signInWithGoogle() {
  try {
    ...
    yield call(getSnapshotFromUserAuthAsync, user);
  } catch (error) {
    yield put(signInFailed(error));
  }
}
...
export function* signInAfterSignUp({ payload: { user, additionalDetails } }) {
  yield call(getSnapshotFromUserAuthAsync, user, additionalDetails);
}
